...
#include <bit>
#define NCMAY_POPCOUNT32(x) static_cast<uint32_t>(std::popcount(x))
#define NCMAY_POPCOUNT64(x) static_cast<uint64_t>(std::popcount(x))
#define NCMAY_CTZ32(x) std::countr_zero(x)
#define NCMAY_CTZ64(x) std::countr_zero(x)
#elif defined(__GNUC__) || defined(__clang__)
#define NCMAY_POPCOUNT32(x) __builtin_popcount(x)
#define NCMAY_POPCOUNT64(x) __builtin_popcountll(x)
#define NCMAY_CTZ32(x) __builtin_ctz(x)
#define NCMAY_CTZ64(x) __builtin_ctzll(x)
...
#define NCMAY_CTZ64(x) _tzcnt_u64(x)
#else
...
struct ChessData {
    ...
    {
        return NCMAY_POPCOUNT32(player1Board & VALID_BOARD_MASK);
    }

    // 返回后手当前在盘存活子数。
    inline uint32_t getPlayer2OnBoardCount() const
    {
        return NCMAY_POPCOUNT32(player2Board & VALID_BOARD_MASK);
    }
    ...
    inline uint64_t getHashHard() const
    {
        ...
        for (uint32_t i = 0; i < NUMBERED_PIECE_COUNT; ++i) {
            ...
            //   0    -> 该编号棋子当前不在盘上
            //   1~24 -> 棋盘点位 index + 1
            const uint32_t player1Pos = player1Piece == 0u ? 0u : (NCMAY_CTZ32(player1Piece) + 1u);
            const uint32_t player2Pos = player2Piece == 0u ? 0u : (NCMAY_CTZ32(player2Piece) + 1u);
