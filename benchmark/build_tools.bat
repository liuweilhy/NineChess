@echo off
rem Build tools: DepthProbe (depth timing), ReplayDebug (dual-model replay), TestMill (mill detection A/B)
rem Result exes: bin\DepthProbe.exe bin\ReplayDebug.exe bin\TestMill.exe
setlocal
...
cl %CFLAGS% /Fo:obj\tools\ depthprobe.cpp replay_debug.cpp test_oldmill.cpp
...
echo Build OK: bin\DepthProbe.exe bin\ReplayDebug.exe bin\TestMill.exe
endlocal
