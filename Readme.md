...
7. 位棋盘 Alpha-Beta 搜索引擎：迭代加深、分片置换表、TT 走法/杀手/历史排序、静默搜索、Aspiration 窗口、重复局面惩罚、点位结构价值评估、PVS 零宽试探。
...
- 置换表：按规则独立、256 分片锁（多线程与双 AI 对弈互不阻塞）、定长桶数组（512K 条，每桶 2 槽，无堆分配、无满表清理打嗝）；表键每节点只计算一次并做雪崩混合，probe/store 共用。
...
- 评估：权重集中于按规则的 `EvalWeights` 权重表（四行初始值相同，调参按行独立）；子力、手牌、**可提三连**（编号规则下剔除已在历史中登记的同编号三连）、活二、机动性、点位结构价值（边中点优于角点；斜线规则下角点更优）、胜利临近压力分（`winPressureWeight`，默认 150；实测 60 局对抗 8:2 压制关闭方，设 0 关闭）、被闷杀压力分与双三连威胁分（默认 0，`vs` 的 `sp`/`ft` 参数可实验）。
...
- `search [d] [t] [h] [r] [g] [th] [s] [pv] [a] [rep] [q] [dd] [pvs]`
  单局面搜索，打印最佳走法、估值、深度、耗时、节点、NPS、TT 统计与根走法分数。
- `match [n] [d] [h] [r] [th] [s] [mp] [dd]`
  同配置自对弈 n 局，报告胜负 / 步数 / 节点 / NPS。
- `vs [n] [d1] [h1] [pv1] [d2] [h2] [pv2] [s] [a] [rep] [wp]`
  不同配置引擎对抗（深度差、哈希模式、点位价值等），用于强度 A/B；奇偶局交替执先，`wp` 只作用于引擎 2。
