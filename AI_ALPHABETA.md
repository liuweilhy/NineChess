...
alphaBetaPruning(depth)
  ...
  ├─ 限时模式（timeLimitMs > 0）：深度上限放开到 kMaxPly-1，时间预算才是
  │   真实约束——迭代一直加深到预算耗尽，中断保留最后一个完整迭代；
  │   请求深度天然成为保底。实测旧实现“到请求深度即停”让 10 秒预算
  │   平均只用到 0.04~2 秒（90%+ 算力浪费），放开后中局实际深度 +3~6 层。
  │   纯深度模式（timeLimitMs == 0）行为不变，确定性测试路径不受影响。
  └─ 迭代加深 1..depth：
...
|---|---|---|---|---|---|---|
| `depth`（`alphaBetaPruning` 入参） | int | Console 8 / GUI 按界面设置 | 1~127（`kMaxPly=128` 封顶） | 每次搜索 | 迭代加深的目标深度 | `dynamicDepth` 开时中局再 +2；**限时模式下 depth 只是保底深度，时间预算才是真实约束，上限放开到 kMaxPly-1（见 §1.1）**；纯深度模式（timeLimitMs=0）不变 |
| `timeLimitMs` | int64 | 0 | 0 = 不限时；>0 = 预算 | 每次搜索 | 预算内尽量完成更深迭代，中断保留上层完整结果 | GUI 恒用（秒×1000，`AiThread`）；Console `search` 默认 0。限时模式下引擎会用满预算（每步耗时≈预算值），GUI 用户想快就调小秒数 |
...
| `dynamicDepth` | bool | true | on/off | 每次搜索 | 中局根局面 depth+2 | +2 写死；限时时只是抬深度天花板 |

### 2.3 随机性（对局多样性，非强度特性）

| 参数 | 类型 | 默认值 | 取值范围 | 作用域 | 作用 | 备注 |
|---|---|---|---|---|---|---|
| `randomness` | uint32 | 0 | 0 = 纯最优；>0 = 启用 | 每次搜索 | 根节点精确打分 + 分差阈值 + softmax 加权选着 | 对外实测有代价：随机 23 胜 vs 纯最优 25~29 胜（80 局/臂，2026-09） |
| `randomGap` | int32 | 60 | ≥0（估值单位） | 每次搜索 | 随机候选与最优的分差阈值 | 带宽大小在噪声范围内，"开不开"才是决定性的 |
| `randomPlies` | int32 | 0 | 0 = 全程随机；>0 = 仅前 N 条命令 | 每次搜索（按根局面命令数） | 开局随机、之后恢复纯最优 | 常用 rP10+g30；0 = 与旧版全程随机行为一致 |
| `seed` | uint64 | 0 | 任意；0 = 每次随机 | 每个根局面 | 固定种子可复现对局 | `vs` 每局用 `seed + g*2 + 1` |

备注 1：根节点上一迭代最佳走法 +20000 置顶，与随机性目标冲突（分差内并列时永远选
上一迭代的走法）；彻底解法是把随机移出引擎（§6 方向 5）。
...
| 排序启发 | 成三 2400 / 活二 240 / 阻断 180 / 线数 48 / 拆三 −160；提子 3000/128/64/32；根最佳 +20000 | `orderKey` | 静态启发量值 |
...
| `randomness` | 0 | —（无"默认开"概念） | `vs` 命令双方固定 randomness=1 |
...
5. 随机性移出引擎：搜索返回带分数的根走法列表，由控制层实现多样性
   （顺带解决根 +20000 置顶与随机的冲突，§1.9）；引擎变为纯确定性强度组件。
...
**2026-09 批次**：限时模式深度放开（用满时间预算，中局 +3~6 层）；浅层剪枝
...
综合效果（同深度单线程 spot）：中局耗时约减半；限时模式中局实际深度 +3~6 层。
